...
function checkInput() {
    if (!creature.alive) {
        return; //return means, just don't do any of the other functions, otherwise, it can do all the stuff below
    }
    ...
    if (mouseIsOverlapping && mouseIsMoving) {
        ...
        creature.boredomLevel += 1; // this counts the level of boredom for every frame that we do not interact with it
